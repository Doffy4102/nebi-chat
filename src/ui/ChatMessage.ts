...
export class ChatMessage {
  ...
  render(messageOrContent: Message | string, role?: string): void {
    ...
    // Support both full Message object and (content, role) shorthand
    let message: Message;
    ...
    if (message.role === "assistant") {
      ...
      MarkdownRenderer.renderMarkdown(message.content, contentEl, "", null as any);
  ...
  renderStreaming(text: string): void {
    ...
    const contentEl = wrapper.createDiv({ cls: "nebi-chat-bubble-content" });
    MarkdownRenderer.renderMarkdown(text, contentEl, "", null as any);
    addCodeBlockCopyButtons(contentEl);
  }
